...
public interface P2PNodeMapper extends BaseMapper<P2PNode> {
    ...
    /**
     * 查询超时的节点（超过指定分钟数未见，或从未上线）
     *
     * @param timeoutMinutes 超时时间（分钟）
     * @return 超时的节点列表
     */
    @Select("SELECT * FROM p2p_node WHERE status = 'ONLINE' AND (last_seen_time IS NULL OR last_seen_time < DATE_SUB(NOW(), INTERVAL #{timeoutMinutes} MINUTE))")
