...
public class BlockchainController {
    ...
    public Result<Block> getLatestBlock() {
        try {
            ...
            if (latestBlock == null) {
                return Result.notFound("区块不存在");
            }
            return Result.success(latestBlock);
        } catch (Exception e) {
            log.error("获取最新区块失败", e);
            return Result.error("获取最新区块失败");
        }
    }
    ...
    public Result<Block> getBlockByIndex(@PathVariable Long index) {
        try {
     ...
     */
    @GetMapping("/validate")
    public Result<Map<String, Object>> validateChain() {
        try {
            boolean isValid = blockchainService.validateChain();
            Map<String, Object> result = new HashMap<>();
            result.put("valid", isValid);
            result.put("chainHeight", blockchainService.getChainHeight());
            return Result.success(result);
        } catch (Exception e) {
            log.error("验证区块链失败", e);
            return Result.error("验证区块链失败");
        }
    }
    ...
    public Result<Map<String, Object>> mine() {
        try {
            ...
            result.put("message", success ? "挖矿成功" : "挖矿失败（可能没有待打包交易）");
            return Result.success(result);
        } catch (Exception e) {
            log.error("手动挖矿失败", e);
            Map<String, Object> result = new HashMap<>();
            result.put("success", false);
            result.put("message", "挖矿失败: " + e.getMessage());
            return Result.success(result);
        }
    }
    ...
    public Result<Map<String, Object>> getMiningStatus() {
        try {
    ...
    public Result<Map<String, Object>> toggleMining(@RequestParam boolean enabled) {
        try {
            if (enabled) {
            ...
            result.put("message", enabled ? "挖矿已启用" : "挖矿已禁用");
            return Result.success(result);
        } catch (Exception e) {
            log.error("切换挖矿状态失败", e);
            return Result.error("切换挖矿状态失败");
        }
    }
}
