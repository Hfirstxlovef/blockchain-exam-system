...
public class ExamPaperController {
    ...
    public Result<Long> createPaper(@RequestBody PaperEncryptedRequest encryptedRequest) {
        try {
            ...
            // 5. 使用多方加密（符合区块链理念）
            // 为教师、系主任、院长分别加密AES密钥，每个人用自己的私钥解密
            paper = paperCryptoService.encryptPaperWithMultiPartyKeys(paper, teacherId);
            log.info("试卷多方加密完成 - 内容哈希: {}", paper.getContentHash());

            // 5.1 数字签名验证（确保创建者身份不可抵赖）
            String privateKey = encryptedRequest.getPrivateKey();
            if (privateKey != null && !privateKey.isEmpty()) {
                // 获取用户公钥
                User user = userService.getUserById(teacherId);
                if (user == null || user.getRsaPublicKey() == null) {
                    return Result.error("用户公钥不存在，请联系管理员");
                }
                String userPublicKey = user.getRsaPublicKey();

                // 生成签名数据
                long timestamp = System.currentTimeMillis();
                String signData = RSAUtil.buildPaperSignData(paper.getContentHash(), teacherId, timestamp);

                // 使用私钥签名
                String signature = RSAUtil.sign(signData, privateKey);

                // 验证签名
                if (!RSAUtil.verify(signData, signature, userPublicKey)) {
                    return Result.error("私钥验证失败，请确认使用正确的私钥文件");
                }

                // 保存签名
                paper.setCreatorSignature(signature);
                log.info("试卷数字签名完成 - 教师ID: {}", teacherId);
            } else {
                log.warn("未提供私钥，跳过数字签名 - 教师ID: {}", teacherId);
            }

            // 6. 保存试卷（content已经被PKI加密，不再需要MybatisCryptoInterceptor处理）
    ...
    @PostMapping("/decrypt")
    @PreAuthorize("hasAnyRole('TEACHER', 'DEPT_ADMIN', 'COLLEGE_ADMIN')")
    public Result<Map<String, Object>> decryptPaper(@RequestBody PaperDecryptRequest request) {
        try {
            ...
            if (!isOwner && !isAdmin) {
                return Result.error("无权解密此试卷");
            }

            // 解密试卷内容
            String decryptedContent = paperCryptoService.decryptPaperContent(paper, privateKey);

            Map<String, Object> data = new java.util.HashMap<>();
            data.put("paperId", paperId);
            data.put("content", decryptedContent);
            data.put("contentHash", paper.getContentHash());
            data.put("verified", true); // 如果解密成功且哈希验证通过

            log.info("试卷解密成功 - 用户: {}, 试卷ID: {}", currentUserId, paperId);
     ...
     * 使用系统私钥解密，无需用户提供私钥
     *
     * @deprecated 不符合区块链透明理念，请使用 /decrypt-with-audit 替代
     */
    @Deprecated
    @ApiOperation("服务端解密试卷内容（审批人专用）- 已废弃")
    @GetMapping("/{id}/decrypt-by-system")
    @PreAuthorize("hasAnyRole('DEPT_ADMIN', 'COLLEGE_ADMIN')")
    public Result<Map<String, Object>> decryptPaperBySystem(@PathVariable Long id) {
        try {
            // 查询试卷
            ExamPaper paper = examPaperService.getById(id);
            if (paper == null) {
            ...
            if (!isAdmin) {
                return Result.error("无权使用服务端解密，请使用私钥解密");
            }

            // 检查是否有系统加密的AES密钥
            if (paper.getSystemEncryptedAesKey() == null || paper.getSystemEncryptedAesKey().isEmpty()) {
                return Result.error("此试卷不支持服务端解密（可能是旧数据），请联系试卷创建者");
            }

            // 使用系统私钥解密
            String decryptedContent = paperCryptoService.decryptPaperBySystemKey(paper);
            ...
            Long currentUserId = SecurityUtil.getCurrentUserId();
            log.info("服务端解密试卷成功 - 审批人: {}, 试卷ID: {}", currentUserId, id);
            return Result.success("解密成功", data);

        } catch (Exception e) {
            log.error("服务端解密试卷失败", e);
            return Result.error("解密失败: " + e.getMessage());
        }
    }
