...
public class ApprovalService {
     ...
     * @param comment    审批意见
     * @param privateKey 用户RSA私钥（Base64编码，用于生成数字签名）
     */
    @Transactional(rollbackFor = Exception.class)
    public void approve(Long paperId, Long approverId, String comment, String privateKey) {
        // 1. 获取审批人信息
        ...
        // 使用用户提供的RSA私钥进行签名
        // 私钥由用户从本地PEM文件导入，确保不可抵赖性
        String signature = RSAUtil.sign(signData, privateKey);

        // 验证签名是否与用户公钥匹配
        String userPublicKey = getPublicKeyForUser(approverId);
        if (userPublicKey == null) {
            throw new BusinessException("审批人公钥不存在，请联系管理员");
        }
        if (!RSAUtil.verify(signData, signature, userPublicKey)) {
            throw new BusinessException("私钥验证失败，请确认使用正确的私钥文件");
        }
        ...
        if (blockchainService != null) {
            try {
                Long txId = blockchainService.submitApprovalToChain(paperId, approverId, "approve", signature);
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void reject(Long paperId, Long approverId, String comment, String privateKey) {
        // 1. 获取审批人信息
        User approver = userService.getUserById(approverId);
        if (approver == null) {
            throw new BusinessException("审批人不存在");
        ...
        if (blockchainService != null) {
            try {
                Long txId = blockchainService.submitApprovalToChain(paperId, approverId, "reject", signature);
    ...
    private String getPublicKeyForUser(Long userId) {
        ...
        String publicKey = user.getRsaPublicKey();
        if (publicKey == null || publicKey.isEmpty()) {
            log.error("用户公钥不存在: userId={}", userId);
            return null;
        }
        return publicKey;
    }
